...
test('shield resolves: held flag set, card discarded', () => {
  ...
  expect(G.abilityHands['0']).toHaveLength(0);
  expect(G.abilityDiscard.map(c => c.id)).toContain('shield-0'); // shield card discarded on success
});
test('guards: not your turn / missing card / unimplemented type / peek without target -> INVALID', () => {
  ...
  // no-mutation-on-reject: none of the four rejected calls may touch G
  expect(G.abilityHands['0']).toHaveLength(2); // hand untouched
  expect(G.abilityDiscard).toHaveLength(0);    // nothing discarded
  expect(G.peekGrants).toEqual({});            // peek-without-target did NOT lazy-create a grant
  expect(G.shields).toEqual({});               // no shield set
});
